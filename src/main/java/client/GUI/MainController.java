...
public class MainController {
    ...
    @FXML
    private TableView<common.UserStats> leaderboardTable;
    @FXML
    private TableColumn<common.UserStats, String> lbNameColumn;
    @FXML
    private TableColumn<common.UserStats, Integer> lbPointsColumn;
    @FXML
    private TableColumn<common.UserStats, Integer> lbGoalsScoredColumn;
    @FXML
    private TableColumn<common.UserStats, Integer> lbGoalsSavedColumn;
    ...
    public void updateLeaderboard(List<?> leaderboard) {
        // Handle both User and UserStats
        if (leaderboard != null && !leaderboard.isEmpty()) {
            if (leaderboard.get(0) instanceof common.UserStats) {
                ObservableList<common.UserStats> leaderboardList = FXCollections.observableArrayList((List<common.UserStats>) leaderboard);
                leaderboardTable.setItems(leaderboardList);
            } else {
                // Convert User to UserStats for backward compatibility
                ObservableList<common.UserStats> statsList = FXCollections.observableArrayList();
                for (Object obj : leaderboard) {
                    if (obj instanceof User) {
                        User user = (User) obj;
                        statsList.add(new common.UserStats(user, 0, 0));
                    }
                }
                leaderboardTable.setItems(statsList);
            }
        }
    }
    ...
    private void initialize() {
        ...
        lbGoalsScoredColumn.setCellValueFactory(new PropertyValueFactory<>("goalsScored"));
        lbGoalsSavedColumn.setCellValueFactory(new PropertyValueFactory<>("goalsSaved"));
