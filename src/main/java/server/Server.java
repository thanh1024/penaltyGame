...
public class Server {
    ...
    public Server() {
        try {
            ...
            // Reset tất cả status về offline khi server khởi động
    ...
    // Reset tất cả users về offline khi server khởi động
    private void resetAllUsersToOffline() {
        try {
            dbManager.resetAllUsersStatus();
            System.out.println("Đã reset tất cả users về trạng thái offline");
        } catch (SQLException e) {
            System.err.println("Lỗi khi reset status users: " + e.getMessage());
            e.printStackTrace();
        }
    }
